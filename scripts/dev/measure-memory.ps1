...
#   PS 5.1 是引擎宿主（$0 依赖、系统内置），其进程基线即 ~73MB WorkingSet，
#   "部署动作内存峰值"按【增量】计量才有意义（宿主基线属运行时本体）。
#   门禁：部署增量 ≤ 45MB（含宿主状态波动余量；实测两轮 39.9 / 40.9MB）；
#        峰值 ≤ 200MB（失控防线）。
...
$okDelta = $deltaMB -le 45
$okPeak = $peakMB -le 200
if ($okDelta -and $okPeak) {
    Write-Host "PASS：增量 ${deltaMB}MB ≤ 45MB 且峰值 ${peakMB}MB ≤ 200MB（§1.4 校准口径）"
    exit 0
}
Write-Host "FAIL：增量 ${deltaMB}MB / 峰值 ${peakMB}MB 超过门禁（增量≤45MB 且峰值≤200MB）"
exit 1